"""
基于arxiv的论文深度搜索和综述生成系统（模块化版本）
该系统将功能拆分为独立的Agent，每个Agent使用LangGraph构建子工作流
"""
...
from typing import List, Dict, Annotated, TypedDict, Optional, Any
...
class RetrievalAgent:
    ...
    def _translate_query_node(self, state: RetrievalState) -> Dict[str, Any]:
        """将中文查询翻译成英文"""
        query = state["topic"]
        # 如果查询已经是英文，直接返回
...
"""
    ...
    def _expand_query_node(self, state: RetrievalState) -> Dict[str, Any]:
        """扩展查询词"""
        query = state["query"]
        prompt = f"""
...
"""
    ...
    def _search_papers_node(self, state: RetrievalState) -> Dict[str, Any]:
        """搜索论文"""
        query = state["query"]
...
class PlannerAgent:
    ...
    def _filter_papers_node(self, state: PlannerState) -> Dict[str, Any]:
        """过滤论文"""
        papers = state["papers"]
        topic = state["topic"]
        ...
        for paper in papers:
            is_relevant = self._evaluate_paper_relevance(paper, topic)
            if is_relevant:
        ...
        if len(relevant_papers) < max_papers * 0.7:  # 如果相关论文少于70%
            print(f"⚠️ 相关论文不足，正在调整关键词重新搜索...")
            additional_papers = self._search_additional_papers(topic, max_papers - len(relevant_papers), papers)
            relevant_papers.extend(additional_papers)

        # 返回相关论文，限制数量
        filtered_papers = relevant_papers[:max_papers]
        return {"filtered_papers": filtered_papers}
...
"""
            ...
            for result in filtered_results[:needed_count]:
                try:
                    ...
                    paper = {
                        ...
                        "entry_id": result.entry_id,
                        "categories": result.categories,
                        "full_content": result.summary  # 简化处理，使用摘要
                    }
                    # 再次检查相关性
    ...
    def _create_workflow(self):
        ...
        # 添加节点
        workflow.add_node("filter_papers", self._filter_papers_node)
        
        # 添加边
        workflow.add_edge(START, "filter_papers")
        workflow.add_edge("filter_papers", END)
        
        return workflow
    
    def plan(self, topic: str, papers: List[Dict]) -> List[Dict]:
        """执行规划"""
...
综合分析报告:"""
    ...
    def _analyze_papers_node(self, state: AnalysisState) -> Dict[str, Any]:
        """分析论文"""
        papers = state["papers"]
        topic = state["topic"]
        ...
        map_reduce_analysis = self._analyze_papers_with_map_reduce(papers)
...
{analysis.content}
    ...
    def _analyze_papers_with_map_reduce(self, papers: List[Dict]) -> str:
        """使用MapReduce方法分析论文"""
        if not papers:
            return "没有找到相关论文。"
...
核心内容: {core_content[:500]}...
    ...
    def _create_workflow(self):
        ...
        # 添加边
        workflow.add_edge(START, "analyze_papers")
        workflow.add_edge("analyze_papers", END)
        
        return workflow
    
    def analyze(self, topic: str, papers: List[Dict]) -> str:
        """执行分析"""
...
class ModularArxivReviewSystem:
    """模块化的arxiv综述生成系统"""
    ...
    def _create_workflow(self):
        ...
        # 定义节点函数
        def retrieval_node(state: SystemState) -> Dict[str, Any]:
            print("🔍 启动论文检索Agent")
            result = self.retrieval_agent.search(state["topic"])
            return {
                "query": result["query"],
                "papers": result["papers"]
            }
        
        def planning_node(state: SystemState) -> Dict[str, Any]:
            print("🧠 启动规划器Agent")
            filtered_papers = self.planner_agent.plan(state["topic"], state["papers"])
            return {"papers": filtered_papers}
        
        def analysis_node(state: SystemState) -> Dict[str, Any]:
            print("📊 启动分析Agent")
            analysis = self.analysis_agent.analyze(state["topic"], state["papers"])
            return {"analysis": analysis}
        
        def review_generation_node(state: SystemState) -> Dict[str, Any]:
            print("📝 启动综述生成Agent")
            review = self.review_agent.generate_review(state["topic"], state["papers"], state["analysis"])
            filename = self.review_agent.save_review(review, state["topic"])
            return {
                "review": review,
                "filename": filename
            }
        ...
        workflow.add_edge("analysis", "review_generation")
    ...
    def visualize_workflow(self):
        ...
        try:
            ...
            # 保存图像到文件
            with open("modular_workflow_graph.png", "wb") as f:
                f.write(image_data)
            print("✅ 模块化工作流图已保存到 modular_workflow_graph.png")
            
            # 在Jupyter Notebook中显示图像
            try:
                display(Image("modular_workflow_graph.png"))
            except:
                print("💡 提示：在Jupyter Notebook中可以显示图像")
                
            return "modular_workflow_graph.png"
        except Exception as e:
...
def run_chat_mode():
    """运行对话模式"""
    print("📚 基于arxiv的论文深度搜索和综述生成系统 (模块化对话模式)")
    print("=" * 60)
    print("支持的命令:")
    print("  - 输入研究主题以生成文献综述")
    print("  - 'help' 查看帮助信息")
    ...
    # 创建综述生成系统
    system = ModularArxivReviewSystem()
    
    # 显示工作流图
    print("🔄 正在生成模块化工作流图...")
    graph_file = system.visualize_workflow()
    if graph_file:
    ...
    while True:
        try:
            ...
            # 生成综述
            print(f"\n🚀 开始处理主题: {user_input}")
            filename = system.generate_review(user_input)
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        filename = system.generate_review(topic)
